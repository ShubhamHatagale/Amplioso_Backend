...
const survey_user_mapping = require('../models/survey_user_mapping.model');
...
exports.getRecords = async (req, res, next) => {
    ...
    try {
        ...
        const Data = await Options.findAll({
            include: [
                { model: survey_user_mapping, as: 'survey_user_mapping', attributes: ['id', 'survey_user_mapping'] }],
...
exports.getRecordsById = async (req, res, next) => {
    try {
        const Data = await Options.findAll({
            include: [
                { model: survey_user_mapping, as: 'survey_user_mapping', attributes: ['id', 'survey_user_mapping'] }],
            where: { survey_user_mapping_id: req.params.Q_id, is_deleted: '0' }
        });
        if (!Data) {
            return res.status(404).json({
                status: 404,
