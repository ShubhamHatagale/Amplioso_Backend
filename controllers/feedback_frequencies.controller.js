...
exports.getRecords = async (req, res, next) => {
  ...
  try {
    const count = await Feedback.findAndCountAll({ where: { is_deleted: 0 } });
    ...
    const Data = await Feedback.findAll({
      include: [
        { model: User, as: 'FeedbackfreqCreated', attributes: ['id', 'first_name'] },
        { model: User, as: 'FeedbackfreqUpdated', attributes: ['id', 'first_name'] }
  ...
  try {
    const Data = await Feedback.findAll({
      include: [
        { model: User, as: 'FeedbackfreqCreated', attributes: ['id', 'first_name'] },
        { model: User, as: 'FeedbackfreqUpdated', attributes: ['id', 'first_name'] }
      ], where: { id: req.params.feedId, is_deleted: 0 }
    });
    if (!Data) {
      return res.status(404).json({
        status: 404,
...
exports.postRecords = async (req, res, next) => {
  ...
  try {
    const feedback = await Feedback.create({
      ...
      feedback_rounds: req.body.feedback_rounds,
...
exports.updateRecords = async (req, res, next) => {
  ...
  try {
    const feedbackdetails = await Feedback.update({
      ...
      { where: { id: req.params.feedId } }, { transaction: t });
    t.commit();
    if (!feedbackdetails) {
      return res.status(200).json({
        status: 404,
