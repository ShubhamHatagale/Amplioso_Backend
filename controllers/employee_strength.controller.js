...
exports.getRecords =async  (req,res,next)=>{
  let queryData = url.parse(req.url, true).query;
    ...
    try {
      const count=await Employee.findAndCountAll({where:{is_deleted:0}});
      ...
      const limit = perPage;
          ...
          const Data = await Employee.findAll({include:[
            {model:User ,as:'EmployeeCreated',attributes:['id','first_name']},
            {model:User ,as:'EmployeeUpdated',attributes:['id','first_name']}
...
exports.getRecordsById=async(req,res,next)=>{
  try {
    const Data = await Employee.findAll({include:[
      {model:User ,as:'EmployeeCreated',attributes:['id','first_name']},
      {model:User ,as:'EmployeeUpdated',attributes:['id','first_name']}
    ],where: {id: req.params.empId,is_deleted:0} });
    if(!Data){            
      return res.status(404).json({
        status: 404,
        message: 'could not find result',              
...
{where: {id: req.params.empId} },{transaction:t});
t.commit();
  if(!employeedetails){
    return res.status(200).json({
      status: 404,
